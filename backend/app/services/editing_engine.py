def generate_edit_plan(video_data, beats):
    ...
    videos_with_faces = [v for v in video_data if v["face_frames"]]
    videos_without_faces = [v for v in video_data if not v["face_frames"]]
    ...
    for i in range(min(len(beats) - 1, 10)):
        ...
        effect = "zoom" if video["face_frames"] else "normal"
